...
class DataGeneration:
    ...
    def _adjust_step_size(self, acceptance_rate):
        
        # Adjust the step size based on the acceptance rate
        if acceptance_rate < 0.2:  # Low acceptance rate: Increase step size
            self.step_size *= 1.1
        elif acceptance_rate > 0.5:  # High acceptance rate: Decrease step size
            self.step_size *= 0.9
    ...
    def _generate_new_points(self):
        ...
        while len(samples) < self.number_of_new_points:
            ...
            # Propose a new sample by adding a random perturbation
            proposed_sample = current_sample + random.uniform(-self.step_size, self.step_size)
