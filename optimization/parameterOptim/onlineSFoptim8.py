...
class optimization():
    ...
    def setInitialConditions(self):
        ...
        if self.time_start != 0:
            ...
            if len(index) > 0:
                time_end_previous = np.max(time_end[index])
    ...
    def optimization(self,inputOutput):
        ...
        def costFunction(sf_selected_value):
            ...
            for i in range(1,len(FR_sciantix)):
                for k in range(len(temperature_sciantix)):
                    if abs(temperature_sciantix[k] - temperature_sciantix[i]) > 20 and k > i:
                        predict_index = k
                        break
                    else:
                ...
                #     error_related[i] = error_integral + 4* error_now + 2*error_future
                # else:
                #     error_related[i] = error_integral + 2* error_now + error_future
                # if FR_interpolated[i] != 0:
...
# Talip1320.setStartEndTime(0,1.134)
