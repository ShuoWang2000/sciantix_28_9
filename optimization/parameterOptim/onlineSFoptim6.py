...
class optimization():
    ...
    def optimization(self,inputOutput):
        ...
        def costFunction(sf_selected_value):
            ...
            for i in range(len(FR_sciantix)):
                if FR_interpolated[i] != 0:
                    if abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i] > 0.1:

                        if FR_interpolated[-1] < FR_sciantix[-1]:
                            if dFR_dt[-1] > 10e-10:
                                
                                error_related[i] = abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i] + abs(dFR_dt[-1] - dFR_dt_sciantix[-1])/dFR_dt[-1]

                            else:
                                error_related[i] = abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i] + 1
                        else:
                            if dFR_dt[i-1] > 10e-10:
                                
                                error_related[i] = abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i] + abs(dFR_dt[i-1] - dFR_dt_sciantix[i-1])/dFR_dt[i-1]
                                
                                # print(dFR_dt[i-1],dFR_dt_sciantix[i-1],error_related[i])
                            else:
                                
                                error_related[i] = abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i] + 1
                            # print(FR_interpolated[i],FR_sciantix[i],error_related[i])
                    else:
                        error_related[i] = abs(FR_interpolated[i] - FR_sciantix[i])/FR_interpolated[i]
                        # print(error_related[i])
                    # print(dFR_dt[i],dFR_dt_sciantix[i],error_related[i])
                else:
                    if FR_sciantix[i] != 0:
                        if dFR_dt[i-1] != 0:
                            # print()
...
# Talip1320.setStartEndTime(3.78,4.94)
